...
GroundStationMobilityModel::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::GroundStationMobilityModel")
	...
	.AddAttribute ("Time",
				   ...
				   DoubleValue (1.0),
				   MakeDoubleAccessor (&GroundStationMobilityModel::m_time),
				   MakeDoubleChecker<double> ())
  ;
...
GroundStationMobilityModel::DoSetPosition (const Vector &position)
{
  ...
  std::cout <<"GroundStationMobilityModel::DoSetPosition  "<< ": ground station # " << current << ": x = " << m_latitude << ", y = " << m_longitude << std::endl;//swd
}
