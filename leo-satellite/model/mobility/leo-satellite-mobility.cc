...
LeoSatelliteMobilityModel::DoSetPosition (const Vector &position)
{
  ...
  if(m_current<=m_nPerPlane*m_numPlanes/2)//psq
  {
	  if(fmod(floor((m_current-1)/(m_nPerPlane/2)),m_nPerPlane/2))
	  {
		  m_latitude = 105-180/(m_nPerPlane/2)/2 - 180/(m_nPerPlane/2)*fmod(m_current - 1, m_nPerPlane/2);  //-30 even   60（odd）
	  }
	  else
	  {
		  m_latitude = 95-180/(m_nPerPlane/2)/2 - 180/(m_nPerPlane/2)*fmod(m_current - 1, m_nPerPlane/2);  //-40偶   50（奇）
      }
  }
  else
  {
	  if(fmod(floor((m_current-1)/(m_nPerPlane/2)),m_nPerPlane/2))
	  {
		  m_latitude = 85-180/(m_nPerPlane/2)/2 - 180/(m_nPerPlane/2)*fmod(m_current - 1, m_nPerPlane/2);  //-50偶   40（奇）
      }
	  else
	  {
		  m_latitude = 75-180/(m_nPerPlane/2)/2 - 180/(m_nPerPlane/2)*fmod(m_current - 1, m_nPerPlane/2);  //-60偶   30（奇）
      }
  }                                                                                                       //psq
  m_longitude = -180 + 360/(m_numPlanes*2)*floor((m_current - 1)/(m_nPerPlane/2));  //-180+60*
