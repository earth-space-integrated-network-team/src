...
void SendOutgoingCallback(std::string context,
							...
							uint32_t interface)
  {
	NS_LOG_UNCOND(
		...
		packet->GetSize()<<" "<<"Destination:"<<
		header.GetDestination()<<" "<<"interface: "<<
		interface<<" "<<
		context);
  }


void TxCallback(std::string context,Ptr< const Packet > packet, Ptr< Ipv4 > ipv4, uint32_t interface)
{
	NS_LOG_UNCOND(
		"+"<<Simulator::Now().GetSeconds()<<" "<<
		packet->GetSize()<<" "<<
...
void RxCallback(std::string context,Ptr< const Packet > packet, Ptr< Ipv4 > ipv4, uint32_t interface)
{
	NS_LOG_UNCOND(
		"-"<<Simulator::Now().GetSeconds()<<" "<<
		packet->GetSize()<<" "<<
...
void DropCallback(std::string context,const Ipv4Header &header,
		          Ptr< const Packet > packet, Ipv4L3Protocol::DropReason reason,
				  Ptr< Ipv4 > ipv4, uint32_t interface)
{
	NS_LOG_UNCOND(
		"Drop"<<Simulator::Now().GetSeconds()<<" "<<
		packet->GetSize()<<" "<<
		reason<<
		context);
}
...
main (int argc, char *argv[])
{
  ...
  LogComponentEnable("UdpEchoClientApplication", LogLevel(LOG_LEVEL_INFO|LOG_PREFIX_NODE));
  LogComponentEnable("UdpEchoServerApplication", LogLevel(LOG_LEVEL_INFO|LOG_PREFIX_NODE));
  LogComponentEnable("GlobalRouteManagerImpl",LogLevel(LOG_LEVEL_INFO|LOG_PREFIX_NODE|LOG_PREFIX_TIME));
  LogComponentEnable("Ipv4L3Protocol",LogLevel(LOG_LEVEL_INFO));
 ...
 // UdpEchoClientHelper echoClient (sat_network.ground_stations.Get(0)->GetDevice(1)->GetAddress(), 9);
  ...
  //sat_network.ground_stations.Get(0)->GetDevice(1)->




  ApplicationContainer clientApps = echoClient.Install (sat_network.ground_stations.Get(0));
//  ApplicationContainer clientApps = echoClient.Install (sat_network.plane[2].Get(3));
...
//
//  Config::Connect(
//   		  "/NodeList/*/$ns3::Ipv4L3Protocol/SendOutgoing",
//   		  MakeCallback(SendOutgoingCallback));
